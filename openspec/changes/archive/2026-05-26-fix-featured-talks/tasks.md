...
## 6. Layout reordering and speaker names

- [x] 6.1 Fetch speakers collection in frontmatter and build slug→name Map (same pattern as Schedule.astro)
- [x] 6.2 Add `getSpeakerNames()` helper function that resolves talk.speakers slugs to display names
- [x] 6.3 Reorder talk card: title leftmost, time/room rightmost on same row (`flex flex-col sm:flex-row justify-between`)
- [x] 6.4 Add "by {speaker name(s)}" line between title row and abstract
