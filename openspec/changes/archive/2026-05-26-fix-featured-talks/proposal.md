...
- **featured-talks-display**: Requirements for the FeaturedTalks component's date display, abstract rendering, title layout, and speaker attribution are being corrected. The existing `FeaturedTalks.astro` component behavior changes but no new spec-level capabilities are introduced.

## Impact

- `src/components/FeaturedTalks.astro` — all fixes (date, abstracts, titles, preposition, layout reordering, speaker names)
- No changes to content files (talk markdown bodies already contain the abstract text; speakers collection resolves slugs to names)
